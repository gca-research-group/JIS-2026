# Inside Hospital Service

Run from this directory with `python3 main.py`. The service binds HTTPS to `127.0.0.1:8101` using `keys/cert.pem` and `keys/priv.pem`. `HOSPITAL_SERVICE_PORT` and `METRICS_FILE` retain their existing meanings; data is stored in `data/hospital_records.json`.
...
The legacy `api/API2.py` and `api/verifyCertificate.py` interfaces are retired. Put this service root on the Python path to import `create_app` from `app` or `post_action` from `service`.
