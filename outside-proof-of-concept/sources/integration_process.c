...
#define HEALTH_REGISTRY_HOST "127.0.0.1"
#define HEALTH_REGISTRY_PORT "8100"
#define HOSPITAL_HOST "127.0.0.1"
#define HOSPITAL_PORT "8101"
#define MESSAGING_HOST "127.0.0.1"
#define MESSAGING_PORT "9100"

static void service_target(const char *srv_id, const char **host, const char **port, const char **endpoint) {
    if (strcmp(srv_id, HEALTH_REGISTRY_SERVICE_ID) == 0) {
        *host = HEALTH_REGISTRY_HOST; *port = HEALTH_REGISTRY_PORT; *endpoint = "/api/request";
    } else if (strcmp(srv_id, HOSPITAL_SERVICE_ID) == 0) {
        *host = HOSPITAL_HOST; *port = HOSPITAL_PORT; *endpoint = "/api/post";
    } else {
        *host = MESSAGING_HOST; *port = MESSAGING_PORT; *endpoint = "/api/post";
    }
}

static char *read_action(const char *srv_id, const char *patient_id) {
    double t0 = now_ms();
    const char *host = NULL, *port = NULL, *endpoint = NULL;
    service_target(srv_id, &host, &port, &endpoint);
    char payload[1024];
    snprintf(payload, sizeof(payload),
             "{\"runId\":\"%s\",\"programId\":\"%s\",\"patientId\":\"%s\",\"puK\":\"program-public-key\",\"signedCert\":\"SIMULATED_BASELINE_CERTIFICATE\",\"environment\":\"outside\"}",
             current_run_id, program_id, patient_id);

    double tr = now_ms();
    char *response = http_post_json(host, port, endpoint, payload);
    ...
    free(response);
    char *data_enc = body ? json_get_string(body, "dataEnc") : NULL;
    free(body);
    double parse_ms = now_ms() - tp;
    if (!data_enc) {
    ...
    metric("read", "read_request_response_ms", request_ms, srv_id);
    metric("read", "read_parse_ms", parse_ms, srv_id);
...
static int write_action(const char *srv_id, const char *data) {
    ...
    char payload[16384];
    snprintf(payload, sizeof(payload),
             "{\"dataEnc\":\"%s\",\"runId\":\"%s\",\"programId\":\"%s\",\"signedCert\":\"SIMULATED_BASELINE_CERTIFICATE\",\"environment\":\"outside\"}",
             data_json, current_run_id, program_id);
    free(data_json);
